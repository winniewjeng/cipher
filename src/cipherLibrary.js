...
function countCipherRows(text, keyLength) {
  return text.length % keyLength === 0
    ? Math.floor(text.length / keyLength)
    : Math.floor(text.length / keyLength) + 1;
}
...
export default {
   ...
   * @return {string}
   */
  generateAlphaNumericKey(minLength, maxLength) {
  ...
  generateNumberKey(len) {
    let key = [];
    while (!Number(key.join(""))) {
      for (let i = 0; i < len; ++i) {
